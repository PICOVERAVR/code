...
sd_command CMD17 = {.command = 0b01010001, .args[0] = 0, 0, 0, 0, .crc = 0};

typedef struct sd_block {
    uint8_t data[512];
} sd_block;
...
void sd_readBlock(uint64_t block_addr, sd_block *rd_block) {
    ...
    for (int i = 0; i < 4; i++) {
        SPI2_Exchange8bit(0x00); //come back to this bit-twiddling crap
    }
...
uint8_t sd_init() {
    ...
    uint8_t result = 0xFF;
    do {
        sd_writeCommand(&ACMD41);
        result = sd_writeCommand(&CMD55);
    } while (result != 0);
